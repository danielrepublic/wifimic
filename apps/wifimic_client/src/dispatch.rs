...
/// initialize diagnostics logging and, on Windows, attach to an inherited
/// console so their output is visible, since this binary is a GUI-subsystem
/// process with no console by default. [`cli::Command::InternalApplyUpgrade`]
/// is the sole exception: its stdout is owned by the elevated handoff
/// script's own capture, so it does not attach a console.
pub(crate) fn dispatch(command: cli::Command) -> Result<(), Box<dyn std::error::Error>> {
    match command {
        cli::Command::RunAudio => {
            let (diagnostics, _startup_rotation) = logging::initialize_diagnostics()?;
            #[cfg(target_os = "windows")]
            super::run_windows_client(diagnostics)?;
